        ...
        {conversations.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white border border-slate-200/80 rounded-3xl">
            <div className="w-16 h-16 rounded-3xl bg-indigo-50 flex items-center justify-center mb-4">
              <MessageCircle size={28} className="text-indigo-500" strokeWidth={1.5} />
            </div>
            ...
            <Link
              href="/search"
              className="mt-6 px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors"
            >
              ไปค้นหาสิ่งของ
            </Link>
          </div>
          ...
          <div className="space-y-2">
            {conversationsWithUsers.map((conv) => (
              <Link
                ...
                className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-100 hover:border-indigo-200 hover:shadow-md transition-all duration-200 active:scale-[0.99] group"
              >
                {/* Avatar */}
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                  conv.unread > 0 ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-500'
                }`}>
                  <User size={20} />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      {conv.unread > 0 ? (
                        <span className="min-w-[20px] h-5 px-1.5 bg-indigo-600 rounded-full text-white text-[10px] flex items-center justify-center font-bold">
                          {conv.unread}
                        </span>
                      ) : (
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                     {conv.itemTitle ? (
                      <p className="text-[10px] font-semibold text-indigo-600 truncate bg-indigo-50 px-2 py-0.5 rounded-md inline-block">
                        📦 {conv.itemTitle}
                      </p>
                    ) : <div />}
